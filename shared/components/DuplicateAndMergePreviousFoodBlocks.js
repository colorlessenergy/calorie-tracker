...
const DuplicateAndMergeFoodBlocksFromPreviousDate = ({
    allFoodBlocks,
    toggleModal,
    setFoodBlocks
}) => {
    ...
    const handlePreviousFoodBlockDateChange = event => {
        if (event.currentTarget.value)
            return setPreviousFoodBlockDate(event.currentTarget.value);
    };
    const handleDuplicateAndMergePreviousDateSubmit = event => {
        ...
        duplicateAndMergeFoodBlocksFromPreviousDate({
            previousDate: previousFoodBlockDate,
            currentDate: date
        });
        ...
        <form onSubmit={handleDuplicateAndMergePreviousDateSubmit}>
            <div className="flex flex-direction-column mb-2">
                <label
                    ...
                    htmlFor="foodBlocksDate"
                >
                    duplicate previous food blocks
                </label>
                <select
